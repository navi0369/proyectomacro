...
CRISIS_YEARS = slice(1982, 1986)   # 1982‑1986 inclusive
POST_YEARS   = slice(1986, 2002)   # 1986‑2002 inclusive
OUTPUT_DIR   = "../../../assets/tesis/neoliberalismo/serie_completa/componentes_pib"
...
ax.set_xlabel("Año")
ax.set_title("Participación Anual de Componentes del PIB (1982‑2002)", fontweight="bold")
...
# Crisis box (82‑86)
ax.text(1.02, 0.56, "Crisis (82‑86)\n" + stats_crisis,
...
# ── 8. Guardar y mostrar ─────────────────────────────────────────────────────
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "participaciones_anuales_pib.png"), dpi=300)
plt.show()

# %%
import pandas as pd
import sqlite3, imgkit, os

# ── Cargar datos ─────────────────────────────────────────────
with sqlite3.connect("../../../db/proyectomacro.db") as conn:
    df_pib = pd.read_sql_query("SELECT * FROM PIB_Real_Gasto", conn, index_col="año")

# ── Generar HTML bonito con pandas Styler ────────────────────
html = (
    df_pib.style
    .set_caption("PIB Real – Componentes")
    .format(precision=2)
    .set_table_styles([{
        'selector': 'th',
...
out_dir = "../../../assets/tesis/neoliberalismo/serie_completa/componentes_pib"
os.makedirs(out_dir, exist_ok=True)
tmp_html = os.path.join(out_dir, "tmp_pib_table.html")
with open(tmp_html, "w") as f:
    f.write(html)

# ── Convertir HTML a imagen con wkhtmltoimage ───────────────
imgkit.from_file(tmp_html, os.path.join(out_dir, "pib_styler_imgkit.png"), options={
    'format': 'png',
    'encoding': "UTF-8",
    'quiet': '',
    'zoom': '1.25'
...
print("✅ Exportado como imagen con éxito")
